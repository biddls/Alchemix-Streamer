...
*/
    ...
    */
    ...
    // this can allow the creation of back pay or to start the stream next saturday
        ...
        gets[msg.sender][_nextID] = stream(_to, _cps, _start, _freq); // need to work on this
...
*/
    function drainStreams(
        ...
        require(length == _IDs.length, "all the arrays must be the same length");
        require(length == _amounts.length, "all the arrays must be the same length");
        ...
        for (uint256 i; i < length; i++){
            ...
            uint256 _amount = (_stream.freq + _stream.sinceLast) <= block.timestamp ?
            (block.timestamp - _stream.sinceLast) * _stream.cps : 0;

            // calls the borrow function in V2
            (bool success, bytes memory returnData) = _amount > 0 ? address(adrAlcV2).call( // if they are goin to get more than 0 coins
    ...
    /*
    if the approval stuff is removed then we dont need this
    */
/*
